...
import {
  setAuthStorage,
  validateByteFormLength,
} from "../../shared/utils/common";
import { useToastsStore } from "../components/common/Toasts";
import { addDoc, collection } from "firebase/firestore";
import { firebaseAuth, initFireStore } from "../../infrastructure/firebase";
...
import { GithubIcon, GoogleIcon } from "../components/ui/svgComponents";
import {
  GithubAuthProvider,
  signInWithPopup,
  GoogleAuthProvider,
  ...
  const submitCertificateNumber = (e) => {
    (async () => {
      ...
      if (isAccess || isAdmin) {
        ...
        if (users.some((user) => user.nickName === trimNickName))
    ...
    <div className="w-full h-full flex flex-col gap-[60px] justify-center items-center">
      ...
      <div className="flex flex-col gap-[40px] text-center mx-auto">
        <div className="text-7xl font-[800] flex gap-[24px]">
          <div>우리의 공간</div>
          <div className="font-[900] text-line text-amber-400">
            {"< Z-Space />"}
          </div>
        </div>
      </div>
      ...
      <ModalStandard
        ...
        contentArea={
          <form
            ...
            onSubmit={submitCertificateNumber}
          >
      ...
      {isDev && (
        <>
          ...
          <button
            type="button"
            className="p-[50px] text-[30px] bg-black text-white"
