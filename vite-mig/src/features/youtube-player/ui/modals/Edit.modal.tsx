...
const EditModal = () => {
  ...
  const submitInputAreaRef = useRef<HTMLDivElement>(null);
  ...
  // submit URL onChange
  const handleSubmitOnChange = ({ target: { value } }: ChangeEvent<HTMLInputElement>) => {
    setCanSubmit(false);
  ...
  const submitURL = async (e: FormEvent) => {
    ...
    // 새로 추가인 경우
    if (!selectedCurrentMusic || Object.keys(selectedCurrentMusic).length === 0) {
      // 신청곡 최대 개수 이상인 경우
      ...
      // 업데이트 요청
      const isUpdate = updateFireStoreData(
  ...
  useEffect(() => {
    if (!isShowEditModal) {
      setSelectedCurrentMusic({} as any);
      setSubmitURLInput("");
    } else if (!isOnce) {
  ...
  useEffect(() => {
    if (
      selectedCurrentMusic &&
      Object.keys(selectedCurrentMusic).length > 0 &&
          ...
          >
            ...
            <header
              className="py-2 px-2 flex justify-between font-bold border-b-2
                mb-2"
            >
              {selectedCurrentMusic && Object.keys(selectedCurrentMusic).length > 0
                ? "신청곡 수정하기"
                : "유튜브 음악 신청하기"}
